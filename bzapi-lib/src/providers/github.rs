...
#[derive(Clone)]
pub struct GithubProviderOptions {
...
impl LikeUserData for GithubUserData {
...
impl LikeSimpleProviderOptions for GithubProvider {
    fn get_id(&self) -> Option<String> {
        Some(Self::id())
    }
...
#[async_trait::async_trait]
impl LikeOauth2Config for GithubProvider {
    fn get_auth_endpoint() -> Option<AuthorisationHandler> {
        let url = Some("https://github.com/login/oauth/authorize".to_string());

        let params = HashMap::new();

        let config = AuthorisationByConfig {
            url,
            params,
            hook: None,
        };
    ...
    async fn fetch_user_data(&self) -> Option<Box<dyn AuthUser>> {
        ...
        // the github api for the user info (internal requests)
        todo!()
    }
...
impl LikeOauth2MetaConfig for GithubProviderOptions {
    fn get_scopes(&self) -> Vec<String> {
        let mut scopes = vec!["read:user".to_string(), "user:email".to_string()];
        if let Some(additional_scopes) = &self.additional_scopes {
            scopes.extend(additional_scopes.clone());
        }
        scopes
    }
}
...
impl GithubProvider {
    pub fn new(options: GithubProviderOptions) -> Result<Self, ProviderError> {
    ...
    pub fn new_from_env() -> Result<Self, ProviderError> {
        ...
        let client = Self::get_client(&GithubProviderOptions {
    ...
    fn get_client(options: &GithubProviderOptions) -> Result<CatchAllOauth2Client, ProviderError> {
